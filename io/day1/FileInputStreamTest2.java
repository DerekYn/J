...
public class FileInputStreamTest2 {
	...
	public static void main(String[] args) {
		...
		// 파일명을 입력 --> 데이터 소스가 된다.
		// C:\iotestdata\애국가2.txt
		
		byte[] dataArr = new byte[1024];
		int inputlengthbyte = 0;
		int countbyte = 0; // byte수 체크용도.
		...
		try {
			...
			/*
			    fist.read(dataArr) 메소드는 해당 파일에서 데이터를 배열 dataArr 크기인 1024 byte씩 읽어서  
			        읽어온 크기를 int 타입으로 리턴해서 inputlengthbyte 에 저장해주고,
			        이어서 읽어온 내용물은 배열 dataArr 에 저장시켜둔다.  
			        만약에 파일의 내용물에서 읽어들일 데이터가 없다라면 -1 값을 리턴시켜준다. 
			        즉, 파일속의 내용물이 끝이 아니라면 계속해서 while{ } 을 실행해라는 말이다. 
			 */
				System.out.write(dataArr, 0, inputlengthbyte); // System.out 이므로 모니터(콘솔화면)에 출력 
