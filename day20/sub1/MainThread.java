/*
    ...
    5) dead
 	...
 	=== *** Main Thread *** ===
 	 시작점(starting point) 역할을 담당하는 main 메소드를 실행시키는 것도 Thread가 담당한다.
 	 스레드는 우선순위(priority)를 가지며 1 부터 10까지를 갖는다.
...
public class MainThread {
	...
	public static void main(String[] args) {
		...
		Thread thr = Thread.currentThread();		// 현재 실행중인 Thread 객체를 얻어온다.
		
		System.out.println("현재 실행중인 Thread 명 : " + thr.getName());	// 스래드의 이름을 얻어온다.
		
		System.out.println("현재 실행중인 Thread의 우선순위 : " + thr.getPriority());		// Thread의 우선순위를 알아보자
		
		System.out.println("Thread가 가질 수 있는 우선순위 최대 값 : " + Thread.MAX_PRIORITY);
		
		System.out.println("Thread가 가질 수 있는 우선순위 최대 값 : " + Thread.MIN_PRIORITY);
		
		System.out.println("Thread에게 부여된 우선순위 기본 값 : " + Thread.NORM_PRIORITY);
