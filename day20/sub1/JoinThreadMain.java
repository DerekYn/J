...
public class JoinThreadMain {
	...
	public static void main(String[] args) {
		...
		join1.setId("cksdud0830");
		join1.setPasswd("qwer1234");
		
		Join2 join2 = new Join2();
		join2.setName("윤찬영");
		join2.setEmail("cksdud7873@gmail.com");
		
		Join3 join3 = new Join3();
		join3.setTel("010-4568-4652");
		join3.setAddr("경기도 용인시 수지구");
		...
		/*thr2.start();
		thr1.start();
		thr3.start();*/		// CPU가 이거 했다가 저거 했다가 막 함.
		
		
		try {
			thr2.start();
			thr2.join();	// thr2 Thread가 join() 메소드를 호출했으므로
							// thr2 Thread가 종료될때 까지 현재 실행중인 다른 Thread는 block 된다.
				
			thr1.start();
			thr1.join();	// thr1 Thread가 join() 메소드를 호출했으므로
							// thr1 Thread가 종료될때 까지 현재 실행중인 다른 Thread는 block 된다.
			
			thr3.start();
			thr3.join();	// thr3 Thread가 join() 메소드를 호출했으므로
							// thr3 Thread가 종료될때 까지 현재 실행중인 다른 Thread는 block 된다.
