...
public class JobApp {
	...
	public void saveJobSeeker(JobSeeker jobSeeker) {
		...
		if(commonCount >= commonArr.length) {
			...
			if (commonCount > 0) {
				for(int i=0; i<commonCount; i++) {
					if ( commonArr[i] == null ) { // 배열에서 삭제된 것이 있는 경우   
						commonArr[i] = jobSeeker;   // 그 배열의 삭제된 방번호에 새로운 구직자를 저장시켜준다.
						flag = 1;
						break;
					}
				}// end of for--------------------------
			...
			if(flag == 0) {
				commonArr[commonCount++] = jobSeeker;	
			}
	...
	public boolean userIdDuplicateCheck(String userid) {
		...
		for (int i=0; i<commonCount; i++) {
			...
			String existUserid = commonArr[i].getId();
			
			if(commonArr[i] instanceof JobSeeker && userid.equals(existUserid)) { 
	...
	public boolean comIdDuplicateCheck(String comid) {
		...
		for (int i=0; i<commonCount; i++) {
			...
			String existComid = commonArr[i].getId();
			
			if(commonArr[i] instanceof Company && comid.equals(existComid)) {  // 기존에 존재하는 회사의 comid 값과 
	...
	public void showAllGujikja() {
		...
		if(commonCount == 0) {
			...
			for(int i=0; i<commonCount; i++) {
				if(commonArr[i] != null && commonArr[i] instanceof JobSeeker) {
					System.out.println(((JobSeeker)commonArr[i]).showJobSeeker());
					System.out.println("\n");
				}
			} // end of for--------------------
		}
	...
	public void showSearchJobSeeker(Scanner sc) {
		...
		try {
			...
			// ==== 나이 검색 ====
			JobSeeker[] ageJobSeekerArr = new JobSeeker[commonArr.length];
			int ageIndex = 0;
			
			for(int i=0; i<commonCount; i++) {
				 if(commonArr[i] == null) continue;
				 
				 if( commonArr[i] instanceof JobSeeker && agemin <= ((JobSeeker)commonArr[i]).getAge() 
						 								&&	((JobSeeker)commonArr[i]).getAge() <= agemax )
					 ageJobSeekerArr[ageIndex++] = (JobSeeker)commonArr[i];
			}// end of for------------------
	...
	public void deleteJobSeeker(Scanner sc) {
		...
		if (commonCount== 0) {
			...
			for (int i=0; i<commonCount; i++) {
				if(commonArr[i] == null) continue;
				if(commonArr[i] instanceof JobSeeker && commonArr[i].getId().equals(userid)) {
					flag = 1; 
	...
	public void updateJobSeeker(Scanner sc) {
		...
		if (flag == 0) {
			...
			((JobSeeker)commonArr[i]).showJobSeeker(); // 변경대상의 기존정보를 보여준다.
