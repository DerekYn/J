...
public class Outer {
	...
	int a = 10;			// 인스턴스 멤버 변수
	static int b = 20;	// 클래스 변수( static 변수 )
	...
	class Inner {
		...
		void innerMethod() {
			System.out.println(">>> 인스턴스 내부 클래스(instance inner class)의 innerMethod() 메소드 호출함");
		}
		
	}
	
	// *** 네비게이터에 가서 Inner 클래스의 이름이 어떻게 생성되었는지 확인하자.
	// 결과 : Outer$Inner.class      $ 표시이면 내부 클래스이다.
	
	
	// *** 스태틱 내부 클래스(static inner class) 생성 ***
	static class StaticInner {
		...
		void staticInner_InstanceMethod() {
			System.out.println(">>> 스태틱 내부 클래스(static inner class)의 staticInner_InstanceMethod() 메소드 호출함!");
		}
		static void staticInner_StaticMethod() {
			System.out.println(">>> 스태틱 내부 클래스(static inner class)의 staticInner_StaticMethod() 메소드 호출함!");
		}
		
		
	}
	// *** 네비게이터에 가서 Inner 클래스의 이름이 어떻게 생성되었는지 확인하자.
	// 결과 : Outer$StaticInner.class      $ 표시이면 내부 클래스이다.
	
	public void test() {
	// Outer 클래스의 메소드 내에서 인스턴스 내부클래스(instance inner class)의 메소드를
	...
	// 2. 약식 표현
		...
		// Outer는 자기자신이므로 생략이 가능하고, this 도 자기자신이므로 생략이 가능하다.
		inObj2.innerMethod();
	}
