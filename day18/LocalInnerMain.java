...
// *** Inner Local class(내부지역클래스) : 메소드안에서 생성되어진 클래스 ***  
...
public class LocalInnerMain {
	...
	//	int num = 100; // 지역변수
		...
		class LocalInner { // local inner class --> named local class 
			...
			//	num = 200;
				// 내부지역클래스에서 클래스 밖 메소드에서 선언되어진 변수 num을
				// 사용하려고 한다. 값을 변경할 수 없다.
				// 그래서 변수 num을 내부지역클래스에서 사용할 수 있는 경우라면
				// final 로 지정해주는 것이 헷갈리지 않고 좋다.
				System.out.println("num => " + num);
		...
		// *** 네비게이터에 가서  LocalInner 클래스의 이름이 어떻게 생성되었는지 확인합니다.
		// 결과 : LocalInnerMain$1LocalInner.class
		// 외부클래스명+$+숫자+로컬클래스명.class 로 생성된다.
		// 여기서 숫자는 인덱스를 의미하는데, 
		// 서로 다른 메소드 내에서 동일한 명칭의 로컬클래스가 존재할 수 있기 때문에
	...
	void exam() {
		...
		// *** 네비게이터에 가서  LocalInner 클래스의 이름이 어떻게 생성되었는지 확인합니다.
		// 결과 : LocalInnerMain$2LocalInner.class 
		LocalInner local = new LocalInner();
		local.info();
		
		// local.innerLocalmethod(); 은 없다.!!!
		/*
		   지역클래스의 객체 생성은 자기(==지역클래스)가 속한 메소드 안에서만 객체생성이 가능하다.
		   다른 메소드내에서는 객체생성이 불가하다.
		   지역클래스가 사용되어지는 범위는 local 변수의 사용범위와 동일하다.
		   해당 블럭을 벗어나서는 사용이 불가능하므로 지역클래스는 자기가 속한 메소드 안에서만
		   객체생성을 할 수 있다. 
		 */
